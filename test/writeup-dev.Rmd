...
* _Note:_ It is highly recommended to pass a "simulation container" into `metric_vs_del()`.  Details / benefits are in the [section below](### Simulator setup (optional; recommended)), but reading this section about `metric_vs_del()` beforehand is helpful.
...
`metric_vs_del()` is named as such because the response metric is passed into `metric_vs_del()` as a _function itself_, for example, `metric_vs_del(graph = g, FUN = igraph::mean_distance(), ...)`.  `FUN` is, in turn, called with the graph in question as an argument, plus any additional arguments.
