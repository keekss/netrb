...
Network robustness can be studied _abstractly_, with generalizations like percolation theory, or more _concretely_ on a given network, by observing how _structural changes_ like vertex deletions affect the network's other attributes -- typically, one or more of its _metrics_.

The `netrb` package allows users to explore of this "concrete" notion of robustness via lightweight _simulations_ on their chosen graph(s) and parameters of interest.
...
By default, results are also plotted during `fun_vs_del()`'s runtime.  Each VDO corresponds with a series in the plot.
